...
class AnelisInstaller
{
	...
	*/
	...
	public static function md5($val, $token)
	{
		$val = md5($val);
		$val = $val.$token.$val.$token.$val;
		$val = md5($val);
		$val = md5($val);
		return $val;
	}
...
if( isset( $_REQUEST['action'] ) )
{
	...
	switch ($action)
	{
		...
		case 'install':
			//Generamos tokens
			$security_token = AnelisInstaller::passwdGen(56);
			$cronjob_token = AnelisInstaller::passwdGen(56);
...
";
			...
			// Perform the query
			$sqlResult = $con->query("ALTER DATABASE ".$mysql_database." CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;");

			if( empty($sqlResult) )
			{
				$error = 'Fallo cambiando CHARSET de la base de datos: ' . $con->connect_error;
				break;
			}
			...
			$sqlInsertAdmin = "INSERT INTO `usuarios_admin` (`id_usuario_admin`, `nombre`, `email`, `password`, `date_created`) VALUES (NULL, '".$_REQUEST['admin_nombre']."', '".$_REQUEST['admin_email']."', '".AnelisInstaller::md5($_REQUEST['admin_pass'], $security_token)."', NOW())";
			...
			$type = 'success';
			$success = 'Instalación realizada correctamente. Debes eliminar la carpeta /install/';
			break;
