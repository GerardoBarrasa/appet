...
class Tools
{
	...
	*/
	 ...
	 * @return string $input 
	 */
	public static function fechaConHora($input, $date_separator='/')
	{
		$input = explode(' ',$input);
		$tiempo= $input[1];
	...
	*/
	 ...
	 * @return string $var
	 */
	public static function urlAmigable($var, $allow_dot=true)
	{  	  
	 ...
	 * @return string
	 */
	public static function cortarString($caracteres, $string, $dots=true)
	{
	 ...
	 * @return boolean
	 */
	public static function searchStrInArray($array, $str)
	{
		foreach ( $array as $ar)
		{
	...
	*/
	 ...
	 * @return html
	 */ 
	public static function breadcrumbs($array, $home_default = true)
	{
	...
	*/
    ...
    public static function resize_image($path, int $width=300, int|string $height='auto'): false|string
    {
        ...
        if (!file_exists($path)) {
            self::logError("Image file does not exist: " . $path);
            return false;
        }
        
        $fileData = file_get_contents($path);
        if ($fileData === false) {
            self::logError("Could not read image file: " . $path);
            return false;
        }
        
        $im = imagecreatefromstring($fileData);
        if ($im === false) {
            self::logError("Could not create image from string for file: " . $path);
            return false;
        }
