...
/**
 * @deprecated hace mucho que no lo usamos ni lo mantenemos
 */
class Validate
{ 
	var $error='';
		...
		foreach( $data as $k => $v )
		{
			if( ( $method == 'post' && isset($_POST[$k]) ) || ( $method == 'get' && isset($_GET[$k]) ) )
			{
				...
				if( $v['validacion'] == 'archivo' )
				{
					...
					if( $name['name'] != '' )
					{
						if( is_uploaded_file($name['tmp_name']) )
						{
							...
							$extension = Tools::getExtension($name['name']);
				...
				else
				{
					...
					if( $method == 'post' )
						$valor = is_array($_POST[$k]) || ( isset($v['html']) && $v['html'] ) ? $_POST[$k] : Tools::getValue($k);
					else if( $method == 'get' )
						$valor = is_array($_GET[$k]) || ( isset($v['html']) && $v['html'] ) ? $_GET[$k] : Tools::getValue($k);		
	...
	*/
	...
	{
		if( isset($_POST[$name]) )
			return Tools::getValue($name) == $value ? 'selected="selected"' : '';	
		else
		{
			if( $edit )
	...
	{
		if( isset($_POST[$name]) )
			return Tools::getValue($name) == $value ? 'checked="checked"' : '';	
		else
		{
			if( $edit )
